...
import { useListTrades, useDeleteTrade, getListTradesQueryKey, useGetStreaks, getGetStreaksQueryKey } from "@workspace/api-client-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { EmotionChip } from "@/components/emotion-chip";
import { PlusCircle, Trash2, ArrowUpRight, ArrowDownRight, RefreshCw, MessageSquare, Flame, Shield, Zap } from "lucide-react";
...
function StreakCard({ icon, label, current, best, unit, color, bgColor, borderColor, description }: StreakCardProps) {
...
function StreaksSkeleton() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
      {[1, 2, 3].map(i => <Skeleton key={i} className="h-44 w-full bg-card" />)}
    </div>
  );
}
...
export default function Dashboard() {
  ...
  const { data: trades, isLoading: tradesLoading } = useListTrades({
    query: { enabled: isAuthenticated, queryKey: getListTradesQueryKey() }
  });

  const { data: streaks, isLoading: streaksLoading } = useGetStreaks({
    query: { enabled: isAuthenticated, queryKey: getGetStreaksQueryKey() }
  });
      ...
      {streaksLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <StreakCard
            ...
            label="Calm Win Streak"
            current={streaks.calmWinStreak.current}
            best={streaks.calmWinStreak.best}
            unit="consecutive calm wins"
            color="text-emerald-400"
            bgColor="bg-emerald-400/15"
            borderColor="border-emerald-400/40"
            description="Consecutive winning trades where your emotion was calm. The discipline of calm compounds."
          />
          <StreakCard
            icon={<Shield className="h-5 w-5" />}
            label="Discipline Streak"
            current={streaks.disciplineStreak.current}
            best={streaks.disciplineStreak.best}
            unit="trades following the plan"
            color="text-blue-400"
            bgColor="bg-blue-400/15"
            borderColor="border-blue-400/40"
            description="Consecutive trades tagged followed_plan. Rules-based trading, every single time."
          />
          <StreakCard
            icon={<Zap className="h-5 w-5" />}
            label="FOMO-Free Days"
            current={streaks.fomoFreeDays.current}
            best={streaks.fomoFreeDays.best}
            unit="trading days without FOMO"
            color="text-violet-400"
            bgColor="bg-violet-400/15"
            borderColor="border-violet-400/40"
            description="Consecutive trading days with zero FOMO-tagged entries. Each day without chasing is a win."
          />
        </div>
      ) : null}
                      ...
                      <>
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <span className="text-xs text-muted-foreground font-mono">
                            Plan: {Math.round(trade.journal.planFollowingScore * 10)}/10
                          </span>
                        </div>
                        <div className="mb-4">
